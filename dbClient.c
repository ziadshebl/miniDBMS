...
int acquireSemaphore(struct clientManagerMsgBuffer toSendMessage, int clientManagerMsgQ,int clientNumber);
void releaseSemaphore(struct clientManagerMsgBuffer toSendMessage, int clientManagerMsgQ,int clientNumber);
...
int main(int argc, char *argv[])
{
    ...
    for (int operation =0; operation < operationCounter; operation++)
    {
        ...
        {
            int operationSucceeded = acquireSemaphore(toSendMessage, clientManagerMsgQ,clientNumber);
            if (operationSucceeded)
            {
                modifyRecord(toSendMessage);
                releaseSemaphore(toSendMessage, clientManagerMsgQ,clientNumber);
